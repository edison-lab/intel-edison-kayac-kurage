...
void readTrim()
{
    ...
    for (i = 0; i < 24; i++)
    {
        data[i] = i2c->readReg(0x88 + i);
    }

    data[24] = i2c->readReg(0xA1);

    for (i = 0; i < 7; i++)
    {
        data[25 + i] = i2c->readReg(0xE1 + i);
    }
...
void readData()
{
    ...
    data[0] = i2c->readReg(0xF7);
    data[1] = i2c->readReg(0xF8);
    data[2] = i2c->readReg(0xF9);
    data[3] = i2c->readReg(0xFA);
    data[4] = i2c->readReg(0xFB);
    data[5] = i2c->readReg(0xFC);
    data[6] = i2c->readReg(0xFD);
    data[7] = i2c->readReg(0xFE);
...
unsigned long int calibration_H(signed long int adc_H)
{
    ...
    v_x1 = (((((adc_H << 14) - (((signed long int) dig_H4) << 20)
        - (((signed long int) dig_H5) * v_x1)) + ((signed long int) 16384)) >> 15)
...
double altitudeCalc(float press,float temp)
{
    float h ;
    h = ((pow( (1013.25/press) , (1.0/5.257) ) - 1) * (temp + 273.15))/0.0065;
    return h ;
}
...
double update()
{
    ...
    altitude  = altitudeCalc(press_act, temp_act);
